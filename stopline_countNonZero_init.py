...
class StopLineCountNoneZero(Calibration):
    ...
    def detect_stopline(self, self.image, high_threshold_value):
        self.image = Calibration.calibrate_image(self.image, self.mtx, self.dist, self.cal_mtx, self.cal_roi)
        stopline_roi, _, _ = self.set_roi(self.image, 250, 350, 10)
        self.image = self.image_processing(stopline_roi, high_threshold_value)
        if cv2.countNonZero(self.image) > 1000:
            print("stopline")
